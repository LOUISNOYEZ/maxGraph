...
export const plugins: GraphPluginConstructor[] = [
  CellEditorHandler,
  SelectionCellsHandler,
...
export const perimetersScenarioSetup = (container: HTMLElement): MyCustomGraph => {
  // Edges have no connection points
  // PolylineShape.prototype.constraints = null; // not useful here

  // Creates the graph inside the given container
  const graphInstance = new MyCustomGraph(container, plugins);
  graphInstance.setConnectable(true);

  // Specifies the default edge style
  graphInstance.getStylesheet().getDefaultEdgeStyle().edgeStyle = 'orthogonalEdgeStyle';
  ...
  graphInstance.batchUpdate(() => {
    const v1 = graphInstance.insertVertex({
      parent,
      id: 'Hello',
      value: 'Hello,',
      position: [20, 20],
    ...
    const v2 = graphInstance.insertVertex({
      parent,
      id: 'World',
      value: 'World!',
      position: [200, 150],
